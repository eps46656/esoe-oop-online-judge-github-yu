...
def index(request):
    ...
    sample_problem = Problem.objects.all()[0]
...
def login(request):
    ...
    if request.user.is_authenticated:
        ...
        return auth_views.LoginView.as_view(template_name='judge/login.html')(request)
...
def logout(request):
    ...
    if not request.user.is_authenticated:
        ...
        return auth_views.LogoutView.as_view(next_page='judge:index')(request)
...
def problem_detail(request, pk):
    ...
    if problem.course == None or not user.profile.courses.filter(pk = problem.course.pk).exists():
        messages.error(request, 'Permission denied.')
        return HttpResponseRedirect(reverse('judge:index'))
